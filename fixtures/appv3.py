...
def to_excel_safe(df: pd.DataFrame, title: str = "Report") -> bytes:
    ...
    for col in df_export.select_dtypes(include=['datetimetz', 'datetime64[ns, tz]']).columns:
        df_export[col] = df_export[col].dt.tz_localize(None)
    # Or simply convert all datetime to naive
...
def load_environment(file_path: str, env_name: str) -> pd.DataFrame:
    ...
    df["expired"] = pd.to_datetime(df["expired"], utc=True).dt.tz_convert(None)  # ← TIMEZONE FIXED!
...
def main():
    ...
    with c3:
        st.markdown(f"<div class='{'metric-bad' if diff > 0 else 'metric-good'} big-font'>{pct:+.1f}%</div>", unsafe_allow_html=True)
    ...
    with tab_health:
        ...
        if "health" in st.session_state:
            ...
            st.dataframe(h.style.apply(lambda r: ["background:#d4edda" if r.Alive else "#f8d7da"] * len(r), axis=1))
            st.download_button("Export Health", to_excel_safe(h), "url_health.xlsx")

    st.markdown("<div class='footer'>Modular • Excel-safe • Dark mode • Filters • Click modal • GitLab CI ready</div>", unsafe_allow_html=True)
