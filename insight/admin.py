...
from .models import Task, ActiveTaskRecord, Queue, Workers
...
class TaskAdmin(admin.ModelAdmin):
    ...
    search_fields = ("uuid", "name", "worker", "root_id", "parent_id")
    readonly_fields = (
        "uuid",
        "sent_at",
        "received_at",
    ...
    fieldsets = (
        ("Basic Info", {"fields": ("uuid", "name", "status", "worker")}),
        (
            "Timing",
            {
                "fields": (
...
class WorkerAdmin(admin.ModelAdmin):
    ...
    actions = [
        ...
        "shutdown_selected_workers",
        "autoscale_selected_workers",
        "grow_pool_selected",
        "shrink_pool_selected",
    ]
    ...
    def restart_selected_workers(self, request, queryset):
        ...
        hostnames = [
            f"celery@{obj.hostname}" for obj in queryset
        ]  # Assuming Celery hostname format
        replies = manager.restart_workers(destination=hostnames, update_orm_after=True)
        if replies:
            self.message_user(
                request,
                f"Restarted {len(queryset)} workers. Replies: {replies}",
                messages.SUCCESS,
            )
        else:
            self.message_user(
                request, f"Failed to restart {len(queryset)} workers.", messages.ERROR
            )

    restart_selected_workers.short_description = (
        "Restart selected workers (pool restart)"
    )
    ...
    def shutdown_selected_workers(self, request, queryset):
        ...
        if replies:
            self.message_user(
                request,
                f"Shutdown initiated for {len(queryset)} workers. Replies: {replies}",
                messages.WARNING,
            )
        else:
            self.message_user(
                request, f"Failed to shutdown {len(queryset)} workers.", messages.ERROR
            )
    ...
    def autoscale_selected_workers(self, request, queryset):
        ...
        if len(queryset) != 1:
            self.message_user(
                request,
                "Please select exactly one worker for autoscaling.",
                messages.ERROR,
            )
            return
        manager = CeleryManager(app)
        hostname = f"celery@{queryset[0].hostname}"
        replies = manager.autoscale_workers(
            min_workers=2, max_workers=8, destination=[hostname]
        )
        if replies:
            self.message_user(
                request,
                f"Autoscaled worker {queryset[0].hostname} to 2-8. Replies: {replies}",
                messages.SUCCESS,
            )
        else:
            self.message_user(
                request,
                f"Failed to autoscale worker {queryset[0].hostname}.",
                messages.ERROR,
            )

    autoscale_selected_workers.short_description = (
        "Autoscale selected worker (min=2, max=8)"
    )
    ...
    def grow_pool_selected(self, request, queryset):
        ...
        if replies:
            self.message_user(
                request,
                f"Grew pool by 1 for {len(queryset)} workers. Replies: {replies}",
                messages.SUCCESS,
            )
        else:
            self.message_user(
                request,
                f"Failed to grow pool for {len(queryset)} workers.",
                messages.ERROR,
            )
    ...
    def shrink_pool_selected(self, request, queryset):
        ...
        if replies:
            self.message_user(
                request,
                f"Shrunk pool by 1 for {len(queryset)} workers. Replies: {replies}",
                messages.SUCCESS,
            )
        else:
            self.message_user(
                request,
                f"Failed to shrink pool for {len(queryset)} workers.",
                messages.ERROR,
            )
...
class QueueAdmin(admin.ModelAdmin):
    ...
    def purge_all_queues(self, request, queryset):
        ...
        self.message_user(
            request, f"Purged {purged_count} tasks from all queues.", messages.WARNING
        )
...
class ActiveTaskRecordAdmin(admin.ModelAdmin):
    ...
    def revoke_selected_tasks(self, request, queryset):
        ...
        if not task_ids:
            self.message_user(
                request, "No active tasks selected for revocation.", messages.ERROR
            )
            return
        manager = CeleryManager(app)
        ...
        if replies:
            self.message_user(
                request,
                f"Revoked {len(task_ids)} tasks. Replies: {replies}",
                messages.SUCCESS,
            )
        else:
            self.message_user(
                request, f"Failed to revoke {len(task_ids)} tasks.", messages.ERROR
            )

    revoke_selected_tasks.short_description = (
        "Revoke selected active tasks (non-terminating)"
    )
    ...
    def mark_as_completed(self, request, queryset):
        ...
        self.message_user(
            request, f"Marked {updated} tasks as completed.", messages.SUCCESS
        )
