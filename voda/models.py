...
class Edge(models.Model):
    ...
    expiry_date = models.DateTimeField(null=True, help_text='expired offer')
...
class Worker(models.Model):
    ...
    last_heartbeat = models.DateTimeField(null=True, editable=False)
...
class TaskState(models.Model):
    task_name = models.CharField(max_length=255, help_text='Celery task name (e.g., update_offer_states)')
    state = models.CharField(max_length=10, choices=TaskStateChoices.choices, default=TaskStateChoices.PENDING,
                             help_text='Current state of the task execution')
    started_at = models.DateTimeField(null=True, blank=True, help_text='When the task started')
    completed_at = models.DateTimeField(null=True, blank=True, help_text='When the task finished')
    result = models.JSONField(null=True, blank=True, help_text='Task result (e.g., {"updated": 5})')
    error = models.TextField(null=True, blank=True, help_text='Error message if failed')
    duration = models.DurationField(null=True, blank=True, help_text='Execution time')
    created_at = models.DateTimeField(auto_now_add=True)
    updated_at = models.DateTimeField(auto_now=True)

    def save(self, *args, **kwargs):
        super().save(*args, **kwargs)
        # Auto-calculate duration if both timestamps are set
